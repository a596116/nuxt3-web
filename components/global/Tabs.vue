...
<template>
  <div ref="tabs" class="tabs">
    <ClientOnly>
      <div
        class="tabs-header bg-primary-700/45 relative flex space-x-6 overflow-hidden rounded-t-lg px-5 py-3 text-sm font-bold text-gray-300">
        <div
        ...
        <span
          ...
          class="absolute top-0 left-0 flex h-full overflow-hidden p-1 py-1.5 transition-all duration-300"
          :style="{ zIndex: 1 }">
          <span class="flex-1 rounded-lg bg-slate-500/40" />
        </span>
      </div>
    </ClientOnly>
    <div
      class="tabs-body relative rounded-b-lg bg-gray-200 text-slate-800 shadow dark:bg-slate-800 dark:text-white">
      <slot />
    </div>
  </div>
</template>
